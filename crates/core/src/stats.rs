...
/// A Ratio Stat's value: `numerator / denominator`, falling back to the
/// raw `numerator` when `denominator` is zero (e.g. KDR before any deaths)
/// rather than dividing by zero. Used for both Lifetime and Session values
/// of every Ratio Stat (KDR, PvP KDR).
pub fn ratio_with_fallback(numerator: f64, denominator: f64) -> f64 {
    if denominator > 0.0 {
...
pub fn compute_lifetime_values(snapshot: &ApiSnapshot) -> HashMap<&'static str, f64> {
    ...
    values.insert("kdr", ratio_with_fallback(kills, deaths));

    let pvp_kills = values.get("pvp_kills").copied().unwrap_or(0.0);
    values.insert("pvp_kdr", ratio_with_fallback(pvp_kills, deaths));
...
mod tests {
    ...
    #[test]
    fn ratio_with_fallback_divides_when_denominator_is_positive() {
        assert_eq!(ratio_with_fallback(100.0, 25.0), 4.0);
    }

    #[test]
    fn ratio_with_fallback_returns_numerator_when_denominator_is_zero() {
        assert_eq!(ratio_with_fallback(7.0, 0.0), 7.0);
    }
